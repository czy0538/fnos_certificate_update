...
DP_Id=$(grep -oP '(?<=DP_Id: ")[^"]+' "$CONFIG_FILE")
DP_Key=$(grep -oP '(?<=DP_Key: ")[^"]+' "$CONFIG_FILE")
...
# 检查是否读取到所有必要的信息
if [ -z "$CERT_NAME" ] || [ -z "$CERT_PATH" ] || [ -z "$BACKUP_DIR" ] || [ -z "$EMAIL" ] || [ -z "$DOMAINS" ] || [ -z "$DP_Id" ] || [ -z "$DP_Key" ] || [ -z "$OLD_CRT" ] || [ -z "$OLD_KEY" ]; then
    echo "配置文件中缺少必需的信息！"
    exit 1
fi
...
# 申请证书并为多个域名生成证书
echo "申请证书：$DOMAINS"
~/.acme.sh/acme.sh --issue --dns dns_dp $DOMAIN_ARGS --force
